...
Neste capítulo, serão explicadas a arquitetura, a metodologia e os
...
Agropecuária (situada na Universidade Estadual de Campinas, UNICAMP).
Nessa reunião, um especialista em desenvolvimento de ontologias da
Embrapa forneceu treinamento sobre a metodologia para definir ontologias,
...
tornar esse conhecimento computável.
...
dessas duas interfaces foi realizada integradamente para validar os
...
específico: sustentabilidade de sistemas produtivos de cana-de-açúcar
...
do sistema. Essa caraterística permite a recuperação da informação
...
\subsection{Classe Production Unit}
...
instâncias \foreignlanguage{english}{Yes} e No.
...
\subsection{Classe Microregion\foreignlanguage{brazil}{ }}
...
consulta SPARQL à \foreignlanguage{english}{DBpedia} e integrados
à ontologia.
...
têm uma propriedade \foreignlanguage{english}{\textit{has value}}\textit{
}que quantifica a sustentabilidade do indicador. Também permitem a
integração de conceitos, inclusive quando pertencem a domínios sem
relação aparente.
...
A propriedade \foreignlanguage{english}{\textit{has value}} usa instâncias
da classe \foreignlanguage{english}{\emph{Value}} para quantificar
a sustentabilidade. Instâncias de \textit{Value} representam as opções
de resposta que os usuários têm para cada indicador\textit{.} Existe
outra propriedade,\textit{ }\foreignlanguage{english}{\textit{has
weight},} que é opcional e estabelece um peso para o indicador\foreignlanguage{english}{\textit{.}}
...
Segundo a descrição do método de avaliação (Apêndice \ref{appendix:Sustainability_Assessment}),
...
sejam do tipo fazendas ou usinas, têm vínculos com pessoas, tanto
...
\subsection*{Modelagem do método de avaliação SustenAgro}

A Figura \ref{fig:Method} mostra um mapa conceitual dos conceitos
envolvidos na avaliação da sustentabilidade, que recebe como entrada
os indicadores selecionados pelos usuários e aplica as fórmulas de
...
e os dados de cada uma foram mudando ao longo do tempo, como resultado
na aplicação de cada técnica, gerando varias definições das Web UI.
Uma vez identificadas as caracterizaras essenciais da interface gráfica,
procedeu-se a definição dos Mockups, que permitiram criar uma representação
visual das interfaces do sistema. 

O desenvolvimento dos \foreignlanguage{english}{Mockups} foi feito
...
e as principais abas, dentre elas, a aba de Ferramenta que permite
iniciar o processo de avaliação de sustentabilidade.
...
Os Mockups representam as \foreignlanguage{english}{widgets,} o que
permitem avaliar a interface gráfica de uma maneira mais próxima à
interface final e assim suportar a sua avaliação por parte dos especialistas
do domínio. 

A Figura \ref{fig:Mockup_indicators} apresenta os passos do processo
de avaliação, na ordem representada pela numeração das abas. A tela
apresentada corresponde ao formulário de cadastro dos indicadores,
que permite cadastrar o valor correspondente a cada indicador.
...
várias mudanças para facilitar a navegabilidade da \foreignlanguage{english}{Web
UI.} Essas mudanças corresponderam à integração de tarefas e a melhora
na apresentação dos formulários e dos resultados de avaliação. Essas
mudanças reestruturaram o \foreignlanguage{english}{look-and-feel}
do SAD. 

\subsection*{Protótipo da Interface Gráfica do SustenAgro}

A partir das melhoras identificadas nas avaliações dos Mockups, foram
identificadas as características finais da Web UI, com as quais foi
desenvolvido um protótipo funcional da Web UI do SustenAgro. Inicialmente,
esse protótipo só permitia interagir com dados simulados.

A Web UI do SAD SustenAgro foi integrada com cada uma das funcionalidades
do SAD em cada ciclo de desenvolvimento, e, atualmente, está disponível
...
Components} específicos. Um deles foi a \foreignlanguage{english}{widget}
...
\par\end{centering}
\caption{Matriz de sustentabilidade mostrando um índice de sustentabilidade
de 2,3 e um índice de eficiência de 2,4.\label{fig:Matriz-de-sustentabilidade-widget}}
\end{figure}
...
componente no sistema SustenAgro com os valores de uma avaliação de
sustentabilidade.
...
\par\end{centering}
\caption{Semáforo de Sustentabilidade mostrando um índice de sustentabilidade
geral de 4 numa escala de -100 até +100. \label{fig:Sem=0000E1foro-de-sustentabilidade}}
...
No comando, a seguir \ref{lis:DSL-EvaluationObject}, o Objeto de
...
\begin{algorithm}[H]
\inputencoding{latin9}\begin{lstlisting}[caption={DSL que define o Evaluation Object },label={lis:DSL-EvaluationObject},breaklines=true]
...
\ref{lis:DSL-Defini=0000E7=0000E3o-de-Features}, são mostradas Features
industriais apenas à usuários de usinas.

\begin{algorithm}
\inputencoding{latin9}\begin{lstlisting}[caption={Defini��o de \foreignlanguage{english}{Features }},label={lis:DSL-Defini=0000E7=0000E3o-de-Features},breaklines=true]
...
No algoritmo \ref{lis:SustenAgro-DSL}, é possível ver o cálculo dos
índices de sustentabilidade e eficiência através das formulas do modelo
usado pelo SustenAgro. É possível usar qualquer comando da linguagem
Groovy ou biblioteca externa. As variáveis guardam os valores de interesse
que serão mostrados no relatório de avaliação. No caso do SustenAgro,
a variável \foreignlanguage{english}{\textit{sustainability}} guarda
o índice de sustentabilidade e a variável \textit{efficiency} guarda
...
\begin{algorithm}[H]
\inputencoding{latin9}\begin{lstlisting}[caption={},label={lis:SustenAgro-DSL},breaklines=true]
data 'data'
report {       
